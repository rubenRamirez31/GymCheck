...
static Future<String?> crearPublicacion(Post post) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final List<Post> posts = postsData.map((data) => Post.fromJson(data)).toList();
      return posts;
    } else {
      throw Exception('Error al obtener todas las publicaciones: ${response.statusCode}');
    }
  } catch (error) {
...
static Future<List<Post>> getPostsByNick(String nick) async {
    String apiUrl = '${Environment.API_URL}/api/social/post/get-posts-by-nick/$nick';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Error al obtener las publicaciones por nick: ${response.statusCode}');
      }
    } catch (error) {
      throw Exception('Error en la solicitud: $error');
    }
...
static Future<List<Post>> getPostsByLugar(String lugar) async {
    String apiUrl = '${Environment.API_URL}/api/social/post/get-posts-by-lugar/$lugar';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Error al obtener las publicaciones por lugar: ${response.statusCode}');
      }
    } catch (error) {
      throw Exception('Error en la solicitud: $error');
    }
...
static Future<List<Post>> getPostsByID(String id) async {
    String apiUrl = '${Environment.API_URL}/api/social/post/get-posts-by-id/$id';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<Post> posts = responseData.map((data) => Post.fromJson(data)).toList();
        return posts;
      } else {
        throw Exception('Error al obtener las publicaciones por ID: ${response.statusCode}');
      }
    } catch (error) {
      throw Exception('Error en la solicitud: $error');
    }
